...
for agenda in final_data:
    base_data = {
        "_id": agenda["_id"],
        "date": agenda["date"],
        "document_page_url": agenda["document_page_url"],
        "title_nl": agenda["title_nl"],
        "title_fr": agenda["title_fr"]
    }
    
    for item in agenda["items"]:
        new_doc = base_data.copy()
        new_doc["item"] = item
        
        agenda_url = new_doc["document_page_url"]
        existing_article = collection.find_one({"document_page_url": agenda_url, "item.nl_text": item["nl_text"]})
