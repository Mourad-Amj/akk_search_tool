...
def get_all_urls(root_url, session):
    ...
    for link in contenu_link:
        href = link.get("href")
        if href.startswith(
            "showpage.cfm?&language=fr&cfm=/site/wwwcfm/qrva/qrvatoc.cfm"
        ):
...
def formatted_data_fr(data):
    return {
        ...
        or data.get("Publication réponse", ""),
...
def formatted_data_nl(data):
    return {
        ...
        "link_to_document": data.get("Publicatie vraag", "")
        or data.get("Publicatie antwoord", ""),
...
def scrapping_data(full_link, session):
    ...
    for header in ("Publication question", "Publication réponse"):
        if header in data_dict:
            ...
            data_dict[
                header
            ] = f"https://www.lachambre.be/QRVA/pdf/{legislature}/{legislature}K0{file}.pdf"
...
def main(language="fr"):
    ...
    with requests.Session() as session:
        ...
        full_links = chain.from_iterable(
            thread_map(partial(scrape_url, session=session), links)
        )
